...
def lookup_exact_glossary_term(
    ...
    row = conn.execute(
        """
        SELECT lt.text AS tgt_text
        FROM term_concept c
        JOIN term_lexeme ls ON ls.concept_id = c.id AND ls.lang=? AND ls.status='approved'
        JOIN term_lexeme lt ON lt.concept_id = c.id AND lt.lang=? AND lt.status='approved'
        WHERE ls.text=? AND (? IS NULL OR c.domain=?)
        ORDER BY lt.is_preferred DESC, lt.priority ASC, lt.id ASC
        LIMIT 1
        """,
        (src_lang, tgt_lang, text, domain, domain),
    ).fetchone()
