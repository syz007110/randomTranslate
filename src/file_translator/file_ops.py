...
def translate_txt(content: str, translate_fn, max_workers: int = 4) -> str:
    ...
    # Heuristic: glossary-dictionary-like text in a single long line.
    # Example: many terms separated by spaces from user-provided dic.txt.
    if len(lines) == 1:
        raw = lines[0].strip()
        tokens = [t for t in raw.split() if t]
        if len(tokens) >= 100:
            translated_tokens = _translate_many(tokens, translate_fn, max_workers=max_workers)
